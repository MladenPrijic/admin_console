...
class Edit extends Component {
  ...
  async componentDidMount() {
    ...
    const resProject = await this.fetch.get(`/api/project/${this.projectId}`);
    let project = { ...this.state.project };
    project.id = resProject.data.project.id;
    project.clientId = resProject.data.project.clientId;
    project.name = resProject.data.project.name;
    project.startDate =
      resProject.data.project.startDate === null
        ? ''
        : resProject.data.project.startDate;
    project.endDate =
      resProject.data.project.endDate === null
        ? ''
        : resProject.data.project.endDate;
    project.notes = resProject.data.project.notes;
    let deploymentsData = resProject.data.project.ProjectUses.map(item => {
      return {
        id: item.id,
        deploymentId: item.Deployment.id,
        deploymentName: item.Deployment.name,
        deploymentType: item.Deployment.type,
        softwareId: item.software.id,
        softwareName: item.software.name,
        softwareVersion: item.software.version,
        status: 'NONE',
      };
    });

    project['deployments'] = deploymentsData;
  ...
  handleOnSave = async e => {
    ...
    try {
      ...
      if (validation.isValid) {
        ...
        await this.fetch.post('/api/project/edit', data);
  ...
  handleOnDelDeployment = (softwareId, deploymentId) => {
    let data = { ...this.state.project };
    const index = data.deployments.findIndex(
      item =>
        item.softwareId === softwareId && item.deploymentId === deploymentId
    );
    if (data.deployments[index].status === 'NONE') {
      data.deployments[index].status = 'DEL';
    } else {
  ...
  render() {
    ...
    const deploymentData = this.state.project.deployments.filter(
            ...
            Cell: props => (
              <ButtonGroup>
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    this.handleOnDelDeployment(
                      props.row._original.softwareId,
                      props.row._original.deploymentId
                    )
                  }>
                  <FaTrash />
                </Button>
        ...
        <AddEditModal
          ...
          <FormGroup>
            ...
            <span className="text-danger">
              {this.state.deployment.deploymentError}
            </span>
          </FormGroup>
          <FormGroup>
            ...
            />
            <span className="text-danger">
              {this.state.deployment.softwareError}
            </span>
          </FormGroup>
        </AddEditModal>
                      ...
                      <Col md="6">
                        <FormGroup>
                          ...
                          <Select
                            ...
                            value={clients.find(
                              option =>
                                option.value === this.state.project.clientId
                            )}
                            onChange={(data, event) =>
                        ...
                        <FormGroup>
                          ...
                          <DatePicker
                            className={{
                              'mr-3 d-block': true,
                              'has-error':
                                this.state.validation.startDate.isInvalid ===
                                true,
                            }}
                            style={ {
                                width:"100%"
